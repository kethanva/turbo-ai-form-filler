...
**Project:** Auto Form Filler (Chrome Extension, Manifest V3)
...
**Before:** The batch prompt (`getBatchAnswers`) and the single-question prompt (`getAnswer`) both contained hardcoded arrays of one real person's biography (`UHG Optum Labs`, `BMC Netreo`, `VMware`, `CGI`, `Tavant`, `Mphasis`, `Liverpool John Moores University`, `K.S.I.T (V.T.U), Bangalore`, dates, locations). For any user, this data was injected into every `[Entry: X]` prompt, meaning:
...
**Verification:** `grep -c "UHG Optum\|BMC Netreo\|Liverpool John\|Kethan" dist/content.bundle.js` returns `0`.
...
```
$ npm run build
> auto-form-filler@1.0.0 build
> tsc && npm run bundle

> auto-form-filler@1.0.0 bundle
